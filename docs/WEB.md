...
- **背题模式**：题干与正确答案同时展示，适合考前快速过题；不计入正确率。顶部可切换两种显示方式 —— **全部选项**（标出正确答案）与**只看答案**（直接给出「题干 → 答案」，不展示干扰项），选择会写入设置，下次进入背题模式默认沿用。
- **选项乱序**：可打乱 A–D 的展示顺序，避免靠位置记答案。
- **判定规则**：与源题库一致 —— 多选题需**选全**所有正确选项才算答对。
- **附图**：78 处题目引用附图，可直接渲染并点击放大查看。
- **键盘操作**：`A`–`D` 选择、`Enter` 提交 / 下一题、`←` `→` 翻题、`S` 收藏、`Esc` 关闭图片放大。
- **底部固定操作条**：练习 / 考试 / 错题重做的「上一题 · 提交 · 下一题」固定在视口底部、贴在移动端标签栏之上，题干或附图再长也不必滚动到页面底部才能点。

### 选项乱序的实现（重要）

乱序只改变**展示顺序**，全站统一约定：

- **持久化与判分一律使用「原始空间」字母**（即 dataset 中的 A/B/C/D 与 `answer`）
- 只有在渲染与点击/键盘交互的边界上才换算到「显示空间」

`lib/question-bank.ts` 提供配套函数：

| 函数 | 用途 |
| --- | --- |
| `optionPermutation(questionId)` | 由题号派生的确定性置换 `map[显示位置] = 原始位置` |
| `optionSpace(question, shuffled)` | 返回 `options` 与两个方向的坐标换算函数 |
| `displayQuestion(question, shuffled)` | 渲染用的题目（选项已重排、答案已重映射） |

这样设计解决三个问题：

1. **顺序稳定**：同一题在练习、考试、错题复盘里顺序一致，刷新后不变（种子来自题号，服务端与客户端算出的结果相同，不会 hydration 不一致）。
2. **切换开关不丢作答**：因为存的是原始空间字母，用户即便在考试中途开关乱序，已选选项仍然指向同一段选项文本（有 e2e 覆盖）。
3. **统计不受影响**：正确率与错题记录都基于原始空间，和乱序开关无关。

题库浏览页（`/browse`）始终按原始顺序渲染 —— 它的职责是与源题库逐字段对照，乱序会破坏这一点。
...
│  ├─ review-list.tsx          逐题复盘列表
│  └─ …                        导航、章节选择器、统计卡、设置面板
├─ lib/
│  ├─ types.ts                 全部数据类型
│  ├─ question-bank.ts         题库访问层（唯一数据入口）+ 选项乱序坐标换算
...
**`npm run test:e2e`（63 项）** —— 用本机 Edge（`puppeteer-core`）真实运行：
题目与选项渲染、键盘快捷键（A–D / Enter / ←→ / S）、判分与错题判定、IndexedDB 落盘（attempts / stats）、附图 `naturalWidth > 0`、错题本收录与展开、「逐题重做」链接与页面、切换题库不串进度、统计页渲染、考试组卷→答题→交卷→成绩页、结果页失分知识点、深色模式切换与刷新保持、Service Worker 注册、**断网后练习页与浏览页仍可用**、**禁用 JS 后浏览页仍有内容**、**下载页链接完整性、PDF 实际下载可解析、处理后 JSON 结构正确、非法 id 被白名单拦截**、**题目卡不再显示源数据提示**，以及本轮新增的：

- **底部操作条**（390×720 手机视口）：完整落在视口内、位于移动端标签栏之上不被遮挡、滚到页面底部位置不变、未滚动即可点「下一题」并成功翻题
- **选项乱序**：开启后顺序变化、四项不丢不重、刷新后顺序稳定、按正确答案作答仍判为正确
- **乱序切换的坐标正确性**：考试中途开启乱序后，已选选项仍指向同一段选项文本（验证「原始空间存盘」的设计）
- **背题模式**：默认展示全部选项、切换后只显示正确答案、刷新后沿用上次选择、切回后同样被记住

当前结果：`25/25` 与 `63/63` 全部通过，`npm run check`（typecheck + lint + build）无错误无警告。
