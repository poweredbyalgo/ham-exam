...
export function ScopePicker({
  ...
  const label =
    scope === "all"
      ? "全部题目"
      : scope.includes(".")
        ? `知识点 ${scope}（${kps.find((k) => k.knowledgePoint === scope)?.count ?? 0} 题）`
        : `${chs.find((c) => c.chapter === scope)?.name ?? `第 ${scope} 章`}`;
    ...
    <div ref={ref} className={`relative ${className}`}>
      ...
      {open && (
        <div
          role="listbox"
          aria-label="选择章节或知识点"
          className="absolute right-0 z-40 mt-1 max-h-[70vh] w-72 overflow-y-auto rounded-xl border border-[var(--border)] bg-[var(--surface)] p-1 shadow-[var(--shadow-lg)]"
        >
          <button
            type="button"
            role="option"
